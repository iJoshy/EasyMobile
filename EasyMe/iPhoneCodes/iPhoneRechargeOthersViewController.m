//
//  iPhoneRechargeOthersViewController.m
//  EasyMobile
//
...
#import "iPhoneRechargeOthersViewController.h"


@implementation iPhoneRechargeOthersViewController
{
    AMSmoothAlertView * alert;
    AMSmoothAlertView * alertCool;
}

@synthesize scrollview, rechargeCode, receiversNo;
...
{
    //NSLog(@"My rechargecode is : %@",rechargecode);
    //NSLog(@"My receiversno is : %@",receiversno);
    
    if ( rechargeCode.text.length == 0 || receiversNo.text.length == 0  )
    {
        [self EmptyField];
    }
    else
    {
        ...
        if ( ( rechargeCode.text.length > 0 && rechargeCode.text.length < 15 ) ||
        ...
        else
        {
            ...
            if ( rechargeCode.text.length > 15  || receiversNo.text.length > 11 )
            {
                [self TooLongField];
            }
            else
...
- (void)actionTaken
{
    ...
    NSString *actionPerformed = @"Recharge Others";
    ...
    else
    {
        ...
        NSString *serviceControllerCode = @"RECHARGE_3RD_PARTY_BY_VOUCHER";
        NSString *prodMainPackage = @"RECHARGE_3RD_PARTY_BY_VOUCHER";
        NSString *prodSubservice = @"RECHARGE_3RD_PARTY_BY_VOUCHER";
        NSString *voucherPIN = rechargeCode.text;
        
        requestData = @{ @"versionNo":versionNo, @"originatingMsisdn":originatingMsisdn, @"destinationMsisdnOne":destinationMsisdnOne, @"osType":osType, @"serviceControllerCode":serviceControllerCode, @"prodMainPackage":prodMainPackage, @"prodSubservice":prodSubservice, @"voucherPIN":voucherPIN };
