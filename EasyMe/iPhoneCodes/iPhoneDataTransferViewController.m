//
//  iPhoneDataTransferViewController.m
//  EasyMobile
//
...
#import "iPhoneDataTransferViewController.h"
#import "AppConstants.h"


@implementation iPhoneDataTransferViewController
{
...
- (void)viewDidLoad
{
    ...
    [self.scrollview setContentSize:CGSizeMake(self.scrollview.frame.size.width, 350.0)];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [tracker set:kGAIScreenName value:@"Data Transfer"];
        ...
        else
        {
            ...
            else
            {
                ...
                if ( securityCode.text.length == 4 && receiversNo.text.length == 11 )
                {
                    ...
                    __weak iPhoneDataTransferViewController *self_ = self;
                    
                    if (!alertCool || !alertCool.isDisplayed)
                    {
                        
                        alertCool = [[AMSmoothAlertView alloc]initFadeAlertWithTitle:@"Confirm your entry" andText:[NSString stringWithFormat:@"\nsecurity code : %@ \nnumber : %@ \nvolume of data : %@",securityCode.text,receiversNo.text, amount.text] andCancelButton:YES forAlertType:AlertInfo];
...
- (void)actionTaken
{
    ...
    else
    {
        ...
        NSString *serviceControllerCode = @"SUBSCRIBER_DATA_TXF";
        NSString *prodMainPackage = @"SUBSCRIBER_DATA_TXF";
        NSString *prodSubservice = @"SUBSCRIBER_DATA_TXF";
        NSString *passKey = securityCode.text;
        NSString *txnAmount = amount.text;
        
        requestData = @{ @"versionNo":versionNo, @"originatingMsisdn":originatingMsisdn, @"destinationMsisdnOne":destinationMsisdnOne, @"osType":osType, @"serviceControllerCode":serviceControllerCode, @"prodMainPackage":prodMainPackage, @"prodSubservice":prodSubservice, @"txnAmount":txnAmount, @"passKey":passKey, };
