//
//  iPhoneEasyExecutiveViewController.m
//  EasyMobile
//
...
#import "iPhoneEasyExecutiveViewController.h"
#import "ShadowButton.h"

@implementation iPhoneEasyExecutiveViewController
{
    AMSmoothAlertView *alert;
    AMSmoothAlertView *alertCool;
}

@synthesize easyExecList, jsonResponse, priceAr, codeAr;
...
- (void)viewDidLoad
{
    ...
    NSData *data = [[NSUserDefaults standardUserDefaults] objectForKey:@"EASYEXECDATA"];
    easyExecList = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    
    NSArray * dataContent = [self easyExecList];
    ...
    while ( i < lengthOfBundles )
    {
        ...
        UILabel *lbl3 = [[UILabel alloc] initWithFrame:CGRectMake(70, 15, 180, 70)];
        [lbl3 setFont:[UIFont fontWithName:@"NeoTechAlt-Medium" size:20.0]];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self.tracker set:kGAIScreenName value:@"EasyBusiness Executive"];
...
-(IBAction)clickedBtn:(id)sender
{
    ...
    __weak iPhoneEasyExecutiveViewController *self_ = self;
    ...
    [ self.tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"Easybusiness Executive"
                                                                action:@"touch"
                                                                 label:[NSString stringWithFormat:@"Easybusiness Executive %@ bundle",priceStr]
                                                                 value:nil] build]];
...
-(void)actionTaken:(NSInteger)number
{
    ...
    productCode = [NSString stringWithFormat:@"%d",((int)number + 1)];
    
    requestData = @{ @"versionNo":versionNo, @"originatingMsisdn":originatingMsisdn, @"osType":osType, @"serviceControllerCode":serviceControllerCode, @"prodMainPackage":prodMainPackage, @"prodSubservice":prodSubservice, @"txnAmount":txnAmount, @"serviceFlag":serviceFlag, @"productCode":productCode };
    
    
    if ([appuser isEqualToString:@"APPLE"])
    {
        PSServerViewController *ps = [PSServerViewController new];
        [ps sendToServer:@"EasyBusiness"];
    }
    else
    {
